...
private val imageStoreExecutor = Executors.newSingleThreadExecutor()
    ...
    ImageManager.create(context).loadImage({ _, drawable, isRequestedDrawable ->
        ...
        imageStoreExecutor.execute {
            try {
                ...
                Log.e(tag, "Failed to save image $fileName", exception)
            }
        }
    }, uri)
}
...
fun Uri.toStringAndSave(godot: Godot, propertyName: String, key: String): String {
    return toStringAndMaybeSave(godot, propertyName, key, true)
}

fun Uri.toStringAndMaybeSave(
    godot: Godot,
    propertyName: String,
    key: String,
    loadImages: Boolean
): String {
    if (!loadImages) return toString()
